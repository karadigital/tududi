...
describe('filterTasksByRecurrence', () => {
    const tasks: Task[] = [
        ...
        createTask({ id: 5, name: 'Monthly weekday', recurrence_type: 'monthly_weekday' }),
        createTask({ id: 6, name: 'Monthly last day', recurrence_type: 'monthly_last_day' }),
    ];

    it('returns all tasks when filter is "all"', () => {
        const result = filterTasksByRecurrence(tasks, 'all');
